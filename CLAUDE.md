...
## What This Is
A single-file HTML/JS/CSS web app (`schedule.html`) for two people — **Alex** and **Jamie** — to manage a shared weekly schedule. No build step; open the file directly in a browser.
...
| `USERS` | Alex, Jamie | The two users |
...
```
Notes are stored separately in `userNotes: { Alex: [], Jamie: [] }` and synced to `schedules/shared-notes` in Firestore.
