...
- `vercel.json` redirects the default Vercel domain to the canonical domain. `config.js` initializes Firebase only on `twosday.dev`, `www.twosday.dev`, or local development hosts, guarding any alternate deployment hostname.
