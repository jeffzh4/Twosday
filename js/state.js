// ── Global app state ──────────────────────────────────────────────────────────
// allData[dateKey][user] = Event[]  e.g. allData["2026-05-16"]["Alex"] = [...]
const allData = {};

let activeUser = 'Alex';
let viewMode = 'week';       // 'day' | 'week' | 'month' | 'year'
let currentDate = new Date();
const userTheme = { Alex: 'dark', Jamie: 'light' };
...
function restoreSnapshot(snap) {
  ...
  userTheme.Alex = snap.userTheme.Alex;
  userTheme.Jamie = snap.userTheme.Jamie;
  applyTheme();
  render();
}
...
function applyParsedData(parsed, applyViewState) {
  ...
  if (parsed.userTheme) {
    if (parsed.userTheme.Alex) userTheme.Alex = parsed.userTheme.Alex;
    if (parsed.userTheme.Jamie) userTheme.Jamie = parsed.userTheme.Jamie;
  }
}
